...
export const UserAddresseCreate = () => {
  ...
  const [countryId, setCountryId] = useState('07d3fe32-37d6-4a6a-ba7e-9a538dae8cb3')
...
const submitHandle = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Client-side validation BEFORE sending to GraphQL
    ...
    if (Object.keys(newErrors).length > 0) {
        setErrors(newErrors)
        return  // Stop here, don't call GraphQL
    }

    setErrors({})
    const variables = { 
        firstName, lastName, companyName, phone, 
        address1, address2, countryId, postcode, 
        state, city, type 
    }

    const result = await addressCreateMutation(variables)
    ...
    if (get(result, 'data.createAddress.id')) {
                ...
                <div className='flex-1 ml-3'>
                  <div className="border rounded">
                    ...
                    <form className="mt-8 space-y-6" onSubmit={(e) => submitHandle(e)} action="#" method="POST">
                        ...
                        <div className='flex w-full'>
<FormSelect options={types} setOnChange={typeOnChange} value={type} />
                        </div>
                        <div className='flex w-full'>
                            <div className='w-1/2'>
