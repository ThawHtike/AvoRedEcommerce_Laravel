...
export const LoginPage = () => {
  ...
  const submitHandle = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    customerLogin(variables).then((mutationResult) => {
		...
		console.log('FULL RESULT:', JSON.stringify(mutationResult))
  console.log('DATA:', mutationResult.data)
  console.log('ERROR:', mutationResult.error)
...
if (!isEmpty(authInfo)) {
  const userInfo = {
    id:             get(authInfo, 'id', ''),
    first_name:     get(authInfo, 'first_name', ''),
    last_name:      get(authInfo, 'last_name', ''),
    email:          get(authInfo, 'email', ''),
    image_path_url: get(authInfo, 'image_path_url', ''),
    addresses:      get(authInfo, 'addresses', []),
  }
  const token = get(authInfo, 'token_info.access_token', '')

  // Save to localStorage directly as backup
  localStorage.setItem('auth_token', token)
  localStorage.setItem('auth_user', JSON.stringify(userInfo))

  dispatch(setAuthInfo({ token, user: userInfo }))
